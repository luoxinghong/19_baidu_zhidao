...
config = {'host': 'localhost', 'port': 3306, 'user': 'root', 'passwd': 'lxh123', 'charset': 'utf8mb4',
...
def handle_url(url, keyword):
    ...
    if title != "":
        ...
        try:
            sql = """insert into baiduzhidao3(url,title,keyword,question,best_answer,best_name,best_date,answers,second_questions) values ("{url}","{title}","{keyword}","{question}","{best_answer}","{best_name}","{best_date}","{answers}","{second_questions}");""".format(
